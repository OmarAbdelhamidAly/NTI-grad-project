...
async def create_virtual_dataset(client: httpx.AsyncClient, db_id: int, sql: str, table_name: str) -> int:
    ...
    payload = {
        ...
        "sql": sql,
        "owners": [1]  # Explicitly assign to admin to avoid AnonymousUser error
    }
...
async def create_chart(client: httpx.AsyncClient, dataset_id: int, slice_name: str, viz_type: str, params: dict, dashboard_id: Optional[int] = None) -> int:
    ...
    # Ensure dataset ID and viz type are injected into params
...
async def create_dashboard(client: httpx.AsyncClient, dashboard_title: str) -> tuple[int, str, str]:
    """Creates a dashboard, enables embedding, and returns (internal_id, internal_uuid, embedded_uuid)."""
    # 1. Create Dashboard
    payload = {
    ...
    embed_payload = {"allowed_domains": []}  # Empty = allow all referrers
    ...
    if not embedded_uuid:
        # Try GET
        resp_get = await client.get(f"{SUPERSET_URL}/api/v1/dashboard/{dashboard_id}/embedded")
        if resp_get.status_code == 200:
            data = resp_get.json()
            embedded_uuid = data.get("result", {}).get("uuid") or data.get("uuid")
            
    if not embedded_uuid:
        embedded_uuid = internal_uuid # Ultimate fallback
